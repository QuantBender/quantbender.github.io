...
genlattice <- function(X0=4, u=1.1, d=.75, N=5) {
  X <- c()
  X[1] <- X0
  count <- 2
...
x<-capture.output(dotlattice(genlattice(N=3, u=2, d=0.5), labels=TRUE))
cat(x, file="lattice1.dot")
```


```{bash, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
dot -Tpng lattice1.dot -o lattice1.png
```

```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics("lattice1.png")
```
...
Consider a bank that has a long position in the lookback option of Example 1.2.4. The
bank intends to hold this option until expiration and receive the payoff $V_3$ . At
time zero, the bank has capital $V_0 = 1.376$ tied up in the option and wants
to earn the interest rate of $25\%$ on this capital until time three (i.e., without
...
three). Specify how the bank's trader should invest in the stock and the money
...
Consider the three-period model of Example 1.2.1, with $S_0 = 4$, $u = 2$, $d = \frac{1}{2}$ and take the interest rate $r = \frac{1}{4}$, so that $\tilde{p}=\tilde{q}=\frac{1}{2}$ - For $n = 0, 1, 2, 3$, define $Y_{n} = \sum_{k=0}^{n}S_{k}$ to be the sum of the stock prices between times zero and n. 
Consider an Asian call option that expires at time three and has strike $K = 4$ (i.e., whose payoff at time three is $(\frac{1}{4}Y_3 - 4)^{+})$. This is like a European call, except the payoff of the option is
based on the average stock price rather than the final stock price. Let $v_{n} (s, y)$
denote the price of this option at time $n$ if $S_n = s$ and $Y_n = y$. In particular,
$v_3 (s, y) = (\frac{1}{4}y - 4)^{+}$.

(i) Develop an algorithm for computing $v_{n}$ recursively. In particular, write a
formula for $v_n$ in terms of $v_{n+1}$.

(ii) Apply the algorithm developed in (i) to compute $v_0(4, 4)$ , the price of the
Asian option at time zero.

(iii) Provide a formula for $\delta_{n} ( s, y )$ , the number of shares of stock that should
be held by the replicating portfolio at time $n$ if $S_n = s$ and $Y_n = y$
